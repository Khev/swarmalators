...
def find_Z(x,y,theta):
    """ Finds time series of the kuramoto order parameter Z  = < e^{i*theta_j)} >_j -- see paper """
    numT, num_osc = x.shape
    Z = 1j*np.zeros(numT)
    for t in range(numT):
        Z[t] = np.sum(np.exp(1j*(theta[t,:]))) / float(num_osc)
    return Z


def find_Z1(x,y,theta):
    """ Finds time series oforder parameter Z  = < r_j^2 e^{i*theta_j)} >_j -- see paper """
    numT, num_osc = x.shape
    Z1 = 1j*np.zeros(numT)
    for t in range(numT):
        dist_sq = x[t,:]**2 + y[t,:]**2 + 0*1j
        Z1[t] = np.sum(dist_sq*np.exp(1j*(theta[t,:]))) / float(num_osc)
    return Z1
